...
namespace TrimbleMonitor.Thunderbolt
{
    ...
    class TsipPacket
    {
        ...
        /// </summary>
        public bool IsComplete => _status == TsipParserStatus.Full;
        ...
        public UInt16 GetNextWord()      // get next two byte (word) field device 
        {
            ...
            {
                buffer[1] = _packetData[_counter++];
                buffer[0] = _packetData[_counter++];
            }
            else
            {
                buffer[0] = _packetData[_counter++];
        ...
        public String GetNextString()
        {
            ...
            var len = _packetData[_counter++];               // length of the string
            for (var i = 0; i < len; i++)
                asciiString.Append(_packetData[_counter++]); // append ascii bytes
                ...
                case TsipParserStatus.DLE2:
                    {
                        switch (newbyte)
                        {
                            ...
                            default: // error: treat as DLE1; start a new report packet
                                Debug.WriteLine("Parse Error: Treat as DLE1");
                                PacketType = newbyte;
                                _length = 0;
                                _status = TsipParserStatus.Data;
                                break;
